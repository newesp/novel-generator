...
**介面語系（Interface Locale）**：
App 自有介面文字與顯示格式所使用的語言；不決定小說內容或 AI 生成內容的語言。
_Avoid_：系統語言、輸出語言、創作語言

**創作語言（Writing Language）**：
建立書籍時選定且之後不可變更、用來約束 AI 產生該書小說相關內容的語言；新書可沿用使用者偏好的預設值，但不受介面語系影響。
_Avoid_：介面語言、Prompt 語言、匯出語言、全域輸出語言、可變書籍設定

**創作內容（Story Content）**：
提供給讀者閱讀的小說相關文字，包含大綱、角色設定、章節、Wiki、對白、旁白與修改建議；其語言由所屬書籍的創作語言決定。
_Avoid_：介面文字、技術提示詞、資料識別碼

**技術提示詞（Technical Prompt）**：
為圖片模型或其他工具最佳化、不是提供給讀者閱讀的機器指令；可使用供應商效果較佳的語言，不受書籍創作語言限制。
_Avoid_：創作內容、內建提示詞、對白、旁白

**內建提示詞（Built-in Prompt）**：
由 App 提供且未經使用者修改的預設 AI 指令，可隨介面語系使用對應的內建版本。
_Avoid_：自訂提示詞、創作語言、固定 Prompt

**自訂提示詞（Custom Prompt）**：
使用者曾修改並保存的 AI 指令；切換介面語系時仍保留原文，不自動翻譯或以內建提示詞覆寫。
_Avoid_：內建提示詞、翻譯版本、暫存 Prompt
