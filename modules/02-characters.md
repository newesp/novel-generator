...
- 角色姓名、背景、性格、關係與 AI 補寫內容依書本 `writingLanguage` 生成或維護。
- 角色面板的標籤、操作、提示與驗證訊息依 `interfaceLocale` 顯示；切換介面語言不翻譯使用者已建立的角色資料。
- 生圖用 visual prompt / negative prompt 可使用 provider 較可靠的技術語言，但不得改變角色設定所屬的故事內容語言。
