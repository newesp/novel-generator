...
| 參考深度       | `buildGenerationPrompt()` 仍保留 `referenceDepth` 參數；目前主 UI 未提供獨立控制 |
...
> **注意（2026-07-27 重複按鈕清理）：** 原章節底部的「↩️ 重新生成」按鈕已移除。因其觸發邏輯與主按鈕「⚡ 快速生成本章」100% 重複，現統一收納於整合 Split Action 下拉選單中。

---

## Multi-Agent 高品質生成與 AI 執行狀態

當選擇「🤖 高品質生成 (Multi-Agent)...」時：
- **預檢與鎖定**：開啟 `MultiAgentPreflightModal` 進行成本與 Agent 模型預檢。確認後建立 `GenerationRun`，全 App 單一 LLM 執行槽啟動並將本章正文切換為 `readOnly` 鎖定保護（仍可選取、查看與複製）。
- **AI Activity Card**：頂部顯示目前 Run 狀態與 Agent 工作卡片，提供關閉 (✕)、取消、人工審核及修復選項。
- **Agent Inspector / 時間軸**：右側 Inspector 的「版本｜Agent」Tab 展現完整時間軸、分步 Token 使用量、Critic 評審證據與候選草稿差異比較。
- **人工審核分支**：Planner 細綱審核、Critic 門檻判定與人工放行 Modal（支援直接採用、傳送 Editor 或人工直接修改）。
...
- 快速生成、Multi-Agent 生成、重新生成、潤色與局部調整都必須遵守書本建立時鎖定的 `Project.writingLanguage`。
- 新章預設標題及 AI 產生的標題依創作語言決定，不依目前介面語言決定。
- 偏好設定中的中、英文內建 Prompt 是可檢視與編輯的起點；執行時仍由不可覆寫的創作語言契約約束輸出。
- `interfaceLocale` 僅控制編輯器、按鈕、狀態、驗證與錯誤訊息，不得在切換後改寫章節正文。
