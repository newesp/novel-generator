...
11. Run a self-review workflow. If Superpowers is available, use the `requesting-code-review` skill to dispatch parallel reviewers for (1) correctness & logic, (2) security & performance, and (3) maintainability, naming, and file locations. Address any Critical or Important findings, then re-run only the passes where such findings remain. Passes that return 0 Critical and 0 Important findings are considered complete and do not need to be re-run. If Superpowers is not available, perform the same three review passes manually or with the available agent tooling.
